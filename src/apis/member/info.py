...
async def get_roles(request: Request):
    ...
    return app.config.user_roles

async def get_ranks(request: Request):
    app: DHApp = request.app
    return app.config.user_ranks

async def get_perms(request: Request):
    app: DHApp = request.app
    return app.config.user_perms
...
async def get_banner(request: Request, response: Response,
    ...
    if os.path.exists(f"/tmp/hub/banner/{app.config.unique_id}_{userid}.png"):
        if time.time() - os.path.getmtime(f"/tmp/hub/banner/{app.config.unique_id}_{userid}.png") <= 600:
            response = StreamingResponse(iter([open(f"/tmp/hub/banner/{app.config.unique_id}_{userid}.png","rb").read()]), media_type="image/jpeg")
            return response
    ...
    try:
        r = await arequests.post(app, app.banner_service_url, data=json.dumps({"company_abbr": app.config.unique_id, \
            "company_name": app.config.org_name, "logo_url": str(app.config.logo_url), "hex_color": app.config.hex_color, \
